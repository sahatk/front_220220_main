...
const fnMakeLi = function(list){ //()내부에 들어오는 이름을 변수명 list로 통칭
    list.innerHTML = insertList;  //내용 삽입
    elCardUl.append(list); //사용
};
...
for(let i = 0; i<cardLen; i+=1){
    let makeLi = document.createElement('li'); // createElement : 요소를 직접 생성
    fnMakeLi(makeLi); //내용 삽입과 사용 기능
